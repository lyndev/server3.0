...
public class QueryActiveCodeTask implements Runnable {
	...
	public QueryActiveCodeTask(S_FC_PlatformInfo platformInfo, String code,
	...
	private static final Logger LOG = Logger.getLogger("QueryActiveCodeTask");
	...
	public void run() {
		...
		if (null == strResponse || strResponse.isEmpty()) {
			mListener.queryActiveCodeCallback(genErrorCodeJson("C1001"), m_code, m_accessToken, m_serverId, m_playerId);
			return;
		}
		...
		try {
			...
			if (obj instanceof JSONObject) {
					mListener.queryActiveCodeCallback(strResponse, m_code, m_accessToken, m_serverId, m_playerId);
					return;
			} else {
				LOG.error("response format not json!");
				mListener.queryActiveCodeCallback(genErrorCodeJson("J1001"), m_code, m_accessToken, m_serverId, m_playerId);
			}
